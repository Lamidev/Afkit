...
//   filters,
//   handleFilter,
//   filterOptions, // This will now contain the dynamically filtered options
//   isMobileFilterOpen,
...
//     if (getSectionId === "category") { // Special handling for category to allow only one selection
//         cpyFilters[getSectionId] = checked ? [getCurrentOption] : [];
//     } else if (getSectionId === "condition") { // Special handling for condition (assuming single select)
//       cpyFilters[getSectionId] = checked ? [getCurrentOption] : [];
//     } else {
...
//     setLocalFilters({});
//     // Reset price range
//     setLocalPriceRange({ min: 0, max: 5000000 });
//     // Apply the empty filters immediately
//     onApplyFilters({}, { min: 0, max: 5000000 });
//     setIsMobileFilterOpen(false);
//     setTimeout(scrollToTop, 100); // Small delay to ensure DOM updates
//   };

//   // Extract filter keys and ensure 'category' is first if it exists
//   const filterKeys = Object.keys(filterOptions).filter(key => Array.isArray(filterOptions[key]) && key !== 'category');
//   if (filterOptions.category) {
//     filterKeys.unshift('category'); // Add 'category' to the beginning if it's available
//   }
//   // Ensure priceRange is handled separately and always shown.
...
//         {/* Price Range Filter - Always shown */}
//         <div>
...
//               <span>
//                 ₦{(localPriceRange?.max ?? 5000000).toLocaleString()}
//               </span>
//             </div>
//           </div>
//         </div>
...
//         {/* All other filters, with 'category' appearing first if present */}
//         {filterKeys.map((keyItem) => {
//           const options = filterOptions[keyItem];
//           if (!Array.isArray(options)) return null; // Should not happen with the new filterKeys logic
...
//                       <Checkbox
//                         id={`${keyItem}-${optionItem.id}`} // Unique ID for checkbox
...
// function ProductFilter({
//   filters,
//   onApplyFilters, // Changed from handleFilter
//   filterOptions,
//   isMobileFilterOpen,
//   setIsMobileFilterOpen, // New prop to close the sheet
//   isFilterLoading,
//   priceRange,
//   setPriceRange,
// }) {
...
//     // Special handling for single-select filters (e.g., category, condition)
...
//   const handleApply = () => {
//     onApplyFilters(localFilters, localPriceRange);
//     setIsMobileFilterOpen(false); // Close the sheet after applying
//     setTimeout(scrollToTop, 100);
//   };

//   const handleReset = () => {
//     setLocalFilters({});
//     setLocalPriceRange({ min: 0, max: 5000000 }); // Ensure max matches your global config
//     onApplyFilters({}, { min: 0, max: 5000000 }); // Apply empty filters
//     setIsMobileFilterOpen(false); // Close the sheet after resetting
//     setTimeout(scrollToTop, 100);
//   };
...
//   return (
//     <div
//       className={`bg-background rounded-lg shadow-sm h-full ${
//         isMobileFilterOpen ? "" : "block w-full"
//       }`}
//     >
//       <div className="p-4 border-b flex items-center justify-between">
//         <h2 className="text-lg font-extrabold">Filters</h2>
//         {isMobileFilterOpen && ( // Only show close button if sheet is open
//           <Button
//             variant="ghost"
//             size="icon"
//             onClick={() => setIsMobileFilterOpen(false)}
...
//       <div className="p-4 space-y-4 overflow-y-auto max-h-[calc(100vh-160px)]"> {/* Added max-height for scrollability in sheet */}
//         {/* Price Range Filter */}
//         <div>
//           <h3 className="text-base font-semibold">Price Range</h3>
//           <div className="mt-2 space-y-2">
//             <Slider
//               min={0}
//               max={5000000} // Ensure this matches your global config
//               step={1000}
//               value={[
//                 localPriceRange?.min ?? 0,
//                 localPriceRange?.max ?? 5000000,
...
//             <Fragment key={keyItem}>
//               <div>
//                 <h3 className="text-base font-semibold capitalize">{keyItem.replace(/-/g, ' ')}</h3> {/* Improved capitalization for display */}
//                 <div className="mt-2 space-y-2">
//                   {options.map((optionItem) => (
//                     <div key={optionItem.id} className="flex items-center gap-2">
//                       <Checkbox
...
//       {/* Apply and Reset buttons - sticky at the bottom */}
//       <div className="p-4 border-t sticky bottom-0 bg-background grid grid-cols-2 gap-2">
//         <Button
//           variant="outline"
//           onClick={handleReset}
