...
function ShoppingCheckout() {
  ...
  const [preferDoorstep, setPreferDoorstep] = useState(false);
  ...
  if (selectedRegion === 'lagos') {
    deliveryMethodText = "Free Doorstep Delivery (Lagos)";
  } else if (selectedRegion === 'south-west') {
    deliveryMethodText = preferDoorstep ? "Doorstep Delivery (Charges apply)" : "Free Delivery to Car Parks (South-West)";
  } else if (selectedRegion === 'south-east-south' || selectedRegion === 'north') {
    deliveryMethodText = preferDoorstep ? "Doorstep Delivery (Charges apply)" : "Free Delivery to Airport (East/North)";
  }
  ...
  function handleInitiatePaystackPayment() {
    ...
    const orderData = {
      ...
      addressInfo: {
        ...
        deliveryPreference: preferDoorstep ? "doorstep" : "hub",
      },
      orderStatus: "pending",
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs font-bold text-slate-700 mt-1 leading-relaxed">
                          {selectedRegion === 'lagos' 
                            ? "Free Doorstep Delivery anywhere in Lagos!"
                            : selectedRegion === 'south-west'
                            ? "Free delivery to major Car Parks. Doorstep delivery is an optional paid upgrade."
                            : "Free delivery to Airport Hubs. Doorstep delivery is an optional paid upgrade."
                          }
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-blue-200/50 flex items-center justify-between">
                         ...
                         >
                           <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                             preferDoorstep ? 'translate-x-6' : 'translate-x-1'
                           }`} />
                         </button>
                      </div>
                    )}
          ...
          <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-xl border border-slate-100 lg:sticky lg:top-28">
            ...
            <div className="mt-8 space-y-4 border-t-2 border-slate-100 pt-6">
              ...
              <div className="flex justify-between items-start text-sm font-bold text-blue-600 uppercase tracking-tight gap-4">
                ...
                <span className="text-right leading-tight text-xs sm:text-sm">{deliveryMethodText}</span>
              </div>
            ...
            <div className="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-3">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className={`text-[11px] font-bold leading-relaxed ${paymentType === "full" && !preferDoorstep ? "text-emerald-600" : "text-orange-600"}`}>
                    {paymentType === "full"
                      ? preferDoorstep && selectedRegion !== 'lagos'
                        ? "PAY RIDER FOR DELIVERY"
                        : "NOTHING — PAID IN FULL"
                      : `₦${(totalCartAmount - 10000).toLocaleString()}${preferDoorstep ? " + FEE TO RIDER" : ""}`}
                  </p>
              ...
              {purchaseIntent !== "personal" && currentSelectedAddress && (
                <div className="flex items-start gap-3 pt-2 border-t border-slate-200">
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase">Recipient</p>
