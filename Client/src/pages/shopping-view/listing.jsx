...
import { sortOptions, categoryOptionsMap, getFilterOptionsForCategory, categorySpecificFilters } from "@/config";
import ProductFilter from "@/components/shopping-view/filter";
import { Button } from "@/components/ui/button";
import {
  DropdownMenu,
...
function ShoppingListing() {
  ...
  const [filters, setFilters] = useState({});
  const [sort, setSort] = useState("latest-arrival"); // Default to latest arrival
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    // Initialize all filters based on categorySpecificFilters for the current category
    // This ensures all relevant filter keys are present in the state, even if empty
    const currentCategoryFilters = categorySpecificFilters[categorySearchParams || "all-products"];
    currentCategoryFilters.forEach(filterKey => {
      if (filterKey !== "priceRange" && filterKey !== "category") { // Handle category and priceRange separately
        initialFilters[filterKey] = [];
      }
    });

    setFilters(initialFilters);
    setSort("latest-arrival"); // Default sort to "latest-arrival" on initial load/category change
    setPriceRange({ min: 0, max: 5000000 });

    dispatch(
      fetchAllFilteredProducts({
        filterParams: initialFilters,
        sortParams: "latest-arrival", // Use "latest-arrival" here too
        priceRange: { min: 0, max: 5000000 },
        signal: abortController.signal
      })
    ).finally(() => {
