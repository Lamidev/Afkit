...
function AdminProducts() {
  ...
  const isFormValid = () => {
    ...
    switch (formData.category) {
      ...
      case 'laptops':
        categorySpecificValid = !!formData.brand && !!formData.storage && !!formData.ram &&
                               !!formData.processor && !!formData.extraFeatures && !!formData.laptopType; // Changed displayType to extraFeatures
        break;
      case 'monitors':
        categorySpecificValid = !!formData.brand && !!formData.screenSize && !!formData.frameStyle &&
                               !!formData.screenResolution && !!formData.ports && !!formData.monitorType; // Added monitorType
        break;
      case 'accessories':
        categorySpecificValid = !!formData.accessoryCategory && !!formData.specificAccessory;
        break;
    ...
    const dynamicControls = addProductFormElements.map(control => {
      ...
      if (newControl.componentType === 'select') {
        if (
          ...
          formData.category === 'laptops' &&
          (newControl.name === 'processor' || newControl.name === 'extraFeatures' || newControl.name === 'laptopType') // Changed displayType to extraFeatures
        ) {
          newControl.options = filterOptions.laptops?.[newControl.name] || [];
        }
        else if (
          formData.category === 'monitors' &&
          (newControl.name === 'brand' || newControl.name === 'screenSize' || newControl.name === 'frameStyle' || newControl.name === 'screenResolution' || newControl.name === 'ports' || newControl.name === 'monitorType') // Added monitorType
        ) {
          newControl.options = filterOptions.monitors?.[newControl.name] || [];
        }
        else if (newControl.name === 'accessoryCategory') {
                  ...
                  <AdminProductTile
                    ...
                    setFormData={(data) => {
                      setFormData({
                        ...
                        ram: data.ram || "",
                        processor: data.processor || "",
                        extraFeatures: data.extraFeatures || "", // Changed from displayType
                        laptopType: data.laptopType || "",
                        screenSize: data.screenSize || "",
                        frameStyle: data.frameStyle || "",
                        screenResolution: data.screenResolution || "",
                        ports: data.ports || "",
                        monitorType: data.monitorType || "", // NEW: Added monitorType
                        accessoryCategory: data.accessoryCategory || "",
                        specificAccessory: data.specificAccessory || "",
                        totalStock: data.totalStock || "",
                        condition: data.condition || "Brand New",
