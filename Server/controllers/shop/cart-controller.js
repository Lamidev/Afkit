...
const addToCart = async (req, res) => {
  try {
    ...
    if (!product) {
      console.error(`[Add To Cart Error] Product ID not found in database: ${productId}`);
  ...
  try {
    ...
    if ((!userId && !sessionId) || !productId || quantity <= 0) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: need either userId or sessionId, productId, and valid quantity",
        required: {
  ...
  try {
    ...
    if ((!userId && !sessionId) || !productId) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: need either userId or sessionId and productId",
        required: {
          identifier: "userId or sessionId",
          productId: true,
        },
